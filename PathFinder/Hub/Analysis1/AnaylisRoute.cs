...
namespace RouteOptimizer.Analysis1
{
    public class AnaylisRoute
    {
        ...
        public static void removeSubRoute(vdDocument doc, Route route, List<Instrument> instruments, TBBOXDestination destination, int u = 3000)
        {
            ...
            foreach (Instrument instrument in instruments)
            {
                ...
                if (minConnector != null) 
                    minConnector.instruments.Add(instrument);
        ...
        public static bool joinLines2(vdDocument doc, Routes routes, List<Instrument> instruments, int maxDistance, RouteInfo routeInfo=null, List<InstrumentInfoEntity> iiet = null)
        {
            ...
            //if (routeInfo != null)
            //{
            //    foreach (Instrument instrument in instruments)// foreach (Instrument instrument in instruments)
            //    {
            //        //   var instrumentj = iie.Instrument;
            //        Connector minConnector = null;
            //        double minDist = double.MaxValue;
            ...
            //            var tb = iiet.Where(x => x.Instrument == instrument).FirstOrDefault().LstInstCableEntity;
            //            bool IsContainDesination = false;
            //            foreach (var tb1 in destinations)
            //            {
            //                var chkExist = tb.Where(x => x.To == tb1.guid.ToString()).Any();
            //                if (chkExist)
            //                {
            //                    IsContainDesination = true;
            //                    break;
            //                }
            //            }

            //            if (IsContainDesination)
            //            {
            //                gPoint gp = connector.line.getClosestPointTo(instrument.centerPoint);
            //                double dist = gp.Distance2D(instrument.centerPoint);
            ...
            //    {
            //        List<TBBOXDestination> destinations = (List<TBBOXDestination>)connector.line.XProperties["Destinations"];
            //        if (destinations.Contains(instrument.destination))
            //        {
            //            gPoint gp = connector.line.getClosestPointTo(instrument.centerPoint);
            //            double dist = gp.Distance2D(instrument.centerPoint);
            //            if (minDist > dist && dist <= maxDistance)
